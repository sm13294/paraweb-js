const { ParallelMapReduceShared } = require("../index");
...
const mapReduce = new ParallelMapReduceShared();
const runner = new PatternTestRunner("MapReduceShared");
...
async function runPerformanceBenchmarks() {
  const summary = await benchmarkRunner.runBenchmark({
    ...
    testFn: async (size: number, threads: number) => {
      const input = __getCachedInput("testMapReduceShared.ts:input:" + String(size), () => (Array.from({ length: size }, (_, i) => (i % 1000) + 1)));
  ...
  benchmarkRunner.saveResults("MapReduceShared");
  benchmarkRunner.generatePlotData("MapReduceShared");
