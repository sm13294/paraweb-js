const { ParallelFarmMP } = require("../index");
...
const farmImpl = new ParallelFarmMP();
const runner = new PatternTestRunner("FarmMP");
...
async function runFunctionalTests() {
  ...
  for (const threads of [1, 2, 4, 8]) {
    await runner.runFunctionalTest(
      ...
      async () => {
        ...
        const output = await farmImpl.farm(
  ...
  // Test 5: Varying computation times (benefits of dynamic distribution)
  for (const threads of [1, 8]) {
    await runner.runFunctionalTest(
...
async function runPerformanceBenchmarks() {
  const summary = await benchmarkRunner.runBenchmark({
    ...
    testFn: async (size: number, threads: number) => {
      const input = __getCachedInput("testFarm.ts:input:" + String(size), () => (Array.from({ length: size }, (_, i) => (i % 1000) + 1000)));
  ...
  benchmarkRunner.saveResults("FarmMP");
  benchmarkRunner.generatePlotData("FarmMP");
