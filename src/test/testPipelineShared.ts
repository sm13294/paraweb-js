...
const pipelineImpl = new ParallelPipelineShared();
const runner = new PatternTestRunner("PipelineShared");
...
async function runPerformanceBenchmarks() {
  const summary = await benchmarkRunner.runBenchmark({
    ...
    sizes: [
      ...
      },
      {
        ...
        size: 10000000,
        description: "Two-stage transform (10M) - sequential stages, parallel per-stage"
      }
    ],
    threadCounts: [1, 2, 4, 8, 16],
    testFn: async (size: number, threads: number) => {
      const input = __getCachedInput("testPipelineShared.ts:input:" + String(size), () => (Array.from({ length: size }, (_, i) => (i % 1000) + 1)));
      const stages = [
  ...
  benchmarkRunner.saveResults("PipelineShared");
  benchmarkRunner.generatePlotData("PipelineShared");
