const { ParallelReduceShared } = require("../index");
...
const runner = new PatternTestRunner("ReduceShared");
...
async function runPerformanceBenchmarks() {
  const summary = await benchmarkRunner.runBenchmark({
    ...
    sizes: [
      ...
      },
      {
        ...
        size: 10000000,
        description: "Compute-reduce (10M) - per-element transform then sum"
      }
    ],
    threadCounts: [1, 2, 4, 8, 16],
    testFn: async (size: number, threads: number) => {
      const input = __getCachedInput("testReduceShared.ts:input:" + String(size), () => (Array.from({ length: size }, (_, i) => (i % 100) + 1)));
      // Associative sum over a light per-element transform; see testReduce.ts.
      const fn = (acc: number, x: number) => acc + x;
  ...
  benchmarkRunner.saveResults("ReduceShared");
  benchmarkRunner.generatePlotData("ReduceShared");
